...
export default function SalesChart({ bookings, numDays }) {
  ...
  // we need to compute an array of data like the one of fakeDate
  // we first create an array of days depending on the filter (last 7/30/90 days)
  const allDates = eachDayOfInterval({
    start: subDays(new Date(), numDays - 1),
    end: new Date(),
  });
  // for each day we need to create an array
  ...
  return (
    <StyledSalesChart>
      ...
      {/* ResponsiveContainer makes the chart repsonsive */}
      <ResponsiveContainer height={300} width="100%">
        <AreaChart data={data}>
          ...
          {/* Tooltip gives us the value of where we hover */}
